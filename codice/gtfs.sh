...
ogr2ogr -f geojson "$cartellaLavoro/output/shapes.geojson" "$cartellaLavoro/output/shapes.csv" -dialect SQLite -sql "SELECT shape_id, MakeLine(MakePoint(CAST(shape_pt_lon AS float),CAST(shape_pt_lat AS float))) FROM shapes GROUP BY shape_id"
